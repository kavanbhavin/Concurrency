...
/*
	Kavan Bhavin kab395@cornell.edu
	Credit to 3410 staff for code in lab3_uart_test.c which has been used in this file
*/
/*------------------------------------------------------------------------
 ...
 *   button).
 *----------------------------------------------------------------------*/
/*Global variable to represent the number of times the timer reaches 65000*/
int overflows = 0;
int main (void) {
	int start, end;
	long diff; 
	...
	TBCTL &= TBCLR;
  	TBCCTL0 = CCIE;
  	TBCCTL0 |= CM0;     /* set to compare mode */
  	TBCTL |= TASSEL_2;  /* set clock to SMCLK */
  	TBCTL |= MC_1;    /* set mode to up mode*/
  	TBCTL |= ID_0;  
  	TBCCR0 = 65000;
 	P1DIR = 0x03;
 	P1OUT = 0x00;
 	...
 	start = TBR;
 	process_start ();
 	/*Save end time of all processes*/
 	end = TBR;
 	/*Calculate time difference.*/
 	diff = (overflows*65000)+(end-start);
 	uart_puts("\n\nAll concurrent processes terminated.\n\n");
 	P1OUT = 0x02;
 	while (1) ;
 	return 0;
}
/*Everytime we reach a max value, increment overflow*/
#pragma vector=TIMERB0_VECTOR
__interrupt void timerb_handler2 (void)
{
	overflows++;
}
